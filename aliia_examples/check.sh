...
c++ -std=c++11 test.cc test.pb.cc -o test_out_annotated `pkg-config --cflags --libs protobuf` -fdata-sections -ffunction-sections -g -Wl,--gc-sections
...
c++ -std=c++11 test.cc test.pb.cc -o test_out_stripped `pkg-config --cflags --libs protobuf`

#run without options
protoc --cpp_out=:. test.proto
c++ -std=c++11 test.cc test.pb.cc -o test_out `pkg-config --cflags --libs protobuf`
